...
function renderMedicineOrders(orders) {
    ...
    if (!orders || orders.length === 0) {
        container.innerHTML = `<div style="text-align:center; padding:40px; background:white; border-radius:15px; border:1px solid #e2e8f0;"><p style="color:#94a3b8; font-weight:600;"><i class="fas fa-pills" style="font-size:30px; margin-bottom:10px;"></i><br>No medicine orders found.</p></div>`;
        return;
    }
    ...
    orders.forEach(order => {
        ...
        if (safeStatus === "Pending") {
            ...
            actionHtml = `<p style="font-size:12px; color:#64748b; margin:0;"><i class="fas fa-info-circle"></i> Pharmacy is reviewing your request.</p>
                          <button onclick="openCancelMedicineModal('${order.order_id}')" style="background:transparent; color:var(--danger); border:1px solid var(--danger); padding:8px 12px; border-radius:8px; font-size:12px; font-weight:bold; cursor:pointer; width:100%; margin-top:10px; transition: 0.2s;">Cancel Order</button>`;
        } 
            ...
            let billBtn = order.medicine_bill ? `<button onclick="window.open('${order.medicine_bill}', '_blank')" style="background:var(--success-light); color:#065f46; border:1px solid #059669; padding:10px; border-radius:8px; font-size:12px; font-weight:bold; cursor:pointer; flex:1;"><i class="fas fa-file-invoice"></i> Bill</button>` : "";
            let rateBtn = order.pharmacy_rating ? `<button disabled style="background:#f1f5f9; color:#94a3b8; border:1px solid #cbd5e1; padding:10px; border-radius:8px; font-size:12px; font-weight:bold; flex:1;"><i class="fas fa-star"></i> Rated</button>` : `<button onclick="openPharmaReviewModal('${order.order_id}')" style="background:var(--warning-light); color:#b45309; border:1px solid #f59e0b; padding:10px; border-radius:8px; font-size:12px; font-weight:bold; cursor:pointer; flex:1;"><i class="fas fa-star"></i> Rate Pharmacy</button>`;
        ...
        let callBtn = order.pharma_mobile && !String(order.pharma_mobile).includes("Hidden") ? `<a href="tel:${order.pharma_mobile}" style="display:block; text-align:center; text-decoration:none; background:var(--primary-light); color:var(--primary); padding:10px; border-radius:8px; font-size:12px; font-weight:bold; margin-top:10px;"><i class="fas fa-phone-alt"></i> Call Pharmacy</a>` : ``;

        let isPickup = (order.order_type === "Collect from Pharmacy" || order.order_type === "Self Pickup");
        let orderTypeDisplay = isPickup 
            ? `<span style="color: #b45309; font-weight: 700; font-size: 10px; background: var(--warning-light); padding: 3px 8px; border-radius: 6px;"><i class="fas fa-store-alt"></i> Self Pickup</span>`
            : `<span style="color: var(--primary); font-weight: 700; font-size: 10px; background: var(--primary-light); padding: 3px 8px; border-radius: 6px;"><i class="fas fa-motorcycle"></i> Home Delivery</span>`;
        ...
        let payStatusBadge = (order.payment_status === "Completed" || order.payment_status === "Paid") 
            ? `<span style="background:var(--success-light); color:#059669; padding:3px 8px; border-radius:6px; font-size:10px; font-weight:800; margin-left:8px;">PAID</span>` 
            : `<span style="background:var(--danger-light); color:#dc2626; padding:3px 8px; border-radius:6px; font-size:10px; font-weight:800; margin-left:8px;">DUE</span>`;

        let pharmaReplyHtml = "";
        if (order.total_mrp && Number(order.total_mrp) > 0) {
            let planDisplay = isUserVip ? `<span style="background:var(--warning-light); color:#b45309; font-size:9px; padding:2px 5px; border-radius:4px; margin-left:5px;">VIP PLAN APPLIED</span>` : `<span style="background:var(--primary-light); color:var(--primary); font-size:9px; padding:2px 5px; border-radius:4px; margin-left:5px;">BASIC PLAN APPLIED</span>`;

            pharmaReplyHtml = `
            <div style="margin-top: 15px; padding-top: 15px; border-top: 1px dashed var(--border);">
                <h5 style="margin:0 0 10px 0; color:var(--text-main); font-size: 13px;">Pharmacy Update</h5>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:10px;">
                    <div style="background:#f8fafc; padding:10px; border-radius:8px; border:1px solid var(--border);">
                        <h6 style="margin:0 0 5px 0; font-size:10px; color:var(--text-light); text-transform:uppercase;">Available</h6>
                        <p style="margin:0; font-size:12px; color:#059669; font-weight:bold;">${order.avail_meds || "Yes"}</p>
                    </div>
                    <div style="background:#f8fafc; padding:10px; border-radius:8px; border:1px solid var(--border);">
                        <h6 style="margin:0 0 5px 0; font-size:10px; color:var(--text-light); text-transform:uppercase;">Not Available</h6>
                        <p style="margin:0; font-size:12px; color:#dc2626; font-weight:bold;">${order.not_avail_meds || "None"}</p>
                    </div>
                </div>
                
                <div style="background:#fff; border:1px solid var(--border); padding:12px; border-radius:12px;">
                    <h6 style="margin:0 0 10px 0; font-size:11px; color:var(--text-main); text-transform:uppercase;">Bill Summary ${planDisplay}</h6>
                    ...
                    <div style="display:flex; justify-content:space-between; font-size:12px; color:#10b981; margin-bottom:5px; font-weight:600;">
                        <span>Discount Applied:</span> <span>- ₹${Number(order.patient_discount).toFixed(2)}</span>
                    </div>
                    <div style="display:flex; justify-content:space-between; font-size:12px; color:#64748b; margin-bottom:10px; border-bottom:1px dashed var(--border); padding-bottom:8px;">
        ...
        let addressHtml = `
        <div style="background:#f8fafc; border:1px solid var(--border); border-radius:12px; padding:12px; margin-bottom:12px; margin-top:12px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                <h6 style="margin:0; color:#475569; font-size:10px; text-transform:uppercase;">${addressLabel}</h6>
            </div>
            ...
            <div style="margin-top: 10px; background: var(--success-light); border: 1px solid #a7f3d0; padding: 10px; border-radius: 8px; color: #065f46;">
                <p style="font-size: 12px; font-weight: 600; margin: 0;">${deliveryIconText} ${formattedDelivery}</p>
            </div>
        </div>`;
        ...
        <div style="background:#ffffff; border:1px solid var(--border); border-radius:16px; padding:18px; margin-bottom:15px; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:12px; border-bottom:1px solid #f1f5f9; padding-bottom:12px;">
                <div>
                    <strong style="color:var(--primary); font-size:14px; font-weight:800;">#${order.order_id}</strong>
                    <div style="margin-top:6px;">${orderTypeDisplay}</div>
                </div>
                <div style="text-align:right;">${badgeHtml}</div>
            </div>
            <p style="font-size:11px; color:#64748b; font-weight:600; margin:0 0 5px 0;"><i class="far fa-calendar-alt"></i> Ordered: ${orderDate}</p>
            
            ${addressHtml}

            <div style="background:#f8fafc; padding:12px; border-radius:10px; font-size:13px; font-weight:600; color:#334155; border:1px solid var(--border);">
                <div style="font-size:10px; color:#94a3b8; margin-bottom:6px; text-transform:uppercase;">Medicine Details:</div>
...
function openConfirmMedicineModal(orderId, prescReq) {
    ...
    if (prescReq === "Yes") {
        prescGroup.style.display = "block"; prescAlert.style.display = "block"; fileInput.required = true;
    } else {
        prescGroup.style.display = "none"; prescAlert.style.display = "none"; fileInput.required = false;
    }
...
async function submitConfirmMedicineForm() {
    ...
    if(fileInput.required && (!fileInput.files || fileInput.files.length === 0)) {
        showToast("Please upload the required prescription.", "error"); return;
    }
    ...
    try {
        let base64File = "";
        if(fileInput.files && fileInput.files.length > 0){ base64File = await getBase64(fileInput.files[0]); }

        const payload = {
            action: "confirmOrderPatient", user_id: localStorage.getItem("bhavya_user_id"),
            order_id: document.getElementById("modalMedOrderId").value, prescription_base64: base64File
        };
    ...
    } catch (error) { showToast("Network error, please try again.", "error"); } finally { btn.innerHTML = "Confirm & Place Order"; btn.disabled = false; }
}
...
function setPharmaRating(stars) {
    ...
    for(let i=0; i<icons.length; i++) { 
        if(i < stars) { icons[i].classList.add("active"); icons[i].style.color = "#f59e0b"; } 
        else { icons[i].classList.remove("active"); icons[i].style.color = "#e2e8f0"; } 
    }
}
...
async function fetchPatientConsults(userId) {
    ...
    try {
        ...
        else { if(container) container.innerHTML = `<div style="text-align: center; padding: 40px; color: #ef4444;"><p>Failed to load consults: ${result.message}</p></div>`; }
    } catch(e) { if(container) container.innerHTML = `<div style="text-align: center; padding: 40px; color: #ef4444;"><p>Network error.</p></div>`; }
}

function renderConsultCards(consults) {
    const container = document.getElementById("patientConsultsContainer"); if (!container) return;
    if (consults.length === 0) { container.innerHTML = `<div style="text-align: center; padding: 40px; color: #94a3b8; font-weight:600; background:white; border-radius:15px; border:1px solid #e2e8f0;"><i class="fas fa-user-md" style="font-size: 30px; margin-bottom: 10px;"></i><p>No doctor consultations found.</p></div>`; return; }
    ...
    consults.forEach(c => {
        ...
        if (safeStatus === "pending" || safeStatus === "approved") {
            cancelBtnHtml = `<button onclick="openConsultCancelModal('${c.appt_id}')" style="background:var(--danger-light); color:var(--danger); border:1px solid var(--danger); padding:10px; border-radius:10px; font-size:12px; font-weight:bold; cursor:pointer; width:100%; margin-top:12px; transition: 0.2s;">Cancel Appointment</button>`;
        }

        if (c.appt_status === "Approved" && c.consult_type === "Online") {
            if (c.handshake_status === "Patient_Ready") {
                joinBtnHtml = `<button onclick="joinVideoCall('${c.meet_link}', '${c.appt_id}')" style="background:var(--warning); color:white; border:none; padding:12px; border-radius:10px; font-size:13px; font-weight:800; cursor:pointer; width:100%; margin-top:12px; animation: pulse 1.5s infinite;"><i class="fas fa-video"></i> Join Video Call</button>`;
            } else if (c.handshake_status === "Doctor_Ready") {
                joinBtnHtml = `<div style="text-align:center; font-size:12px; color:#059669; margin-top:12px; padding: 12px; background: var(--success-light); border-radius: 10px; font-weight:700;"><i class="fas fa-stethoscope"></i> Doctor is waiting. Please click "I Am Ready" from the popup.</div>`;
            } else {
                joinBtnHtml = `<div style="text-align:center; font-size:11px; color:var(--text-light); margin-top:12px; padding: 12px; background: var(--bg-color); border-radius: 10px; font-weight:500;">Video call button will appear here when the doctor is ready.</div>`;
            }
        } 
        
        else if (safeStatus.includes("cancel") || safeStatus === "cancelled") { 
            badgeClass = "status-danger"; statusText = "Cancelled"; 
            let reasonHtml = (c.cancel_reason && c.cancel_reason !== "Payment Not Received") ? `<div style="background:var(--danger-light); color:#991b1b; padding:10px; border-radius:8px; font-size:12px; margin-top:10px; border:1px solid #fecaca; font-weight:500;"><strong>Cancel Reason:</strong> ${c.cancel_reason}</div>` : "";
            let claimHtml = "";
            if (c.cancel_reason === "Payment Not Received" || c.refund_status === "No Refund (Not Paid)") {
                ...
                claimHtml = `<button onclick="openClaimRefundModal('${c.appt_id}')" style="background:var(--warning); color:white; border:none; padding:12px; border-radius:10px; font-size:13px; font-weight:800; cursor:pointer; width:100%; margin-top:12px; animation: pulse 2s infinite;"><i class="fas fa-hand-holding-usd"></i> Claim Refund Now</button>`;
            } 
            else if (c.refund_status && c.refund_status !== "No Refund (Not Paid)") {
                claimHtml = `<div style="font-size:12px; color:#065f46; margin-top:10px; font-weight:700; background:var(--success-light); padding:10px; border-radius:8px; text-align:center;">Refund Status: ${c.refund_status}</div>`;
            }
            ...
            if (c.prescription_link) {
                let valText = c.validity_days ? `<br><span style="font-size:9px; color:var(--text-light); font-weight:600;">Valid for ${c.validity_days}</span>` : "";
                rxHtml = `<div style="flex:1; text-align:center;"><button onclick="window.open('${c.prescription_link}', '_blank')" style="width:100%; background:var(--success-light); color:#065f46; border:1px solid #10b981; padding:10px; border-radius:10px; font-weight:800; font-size:12px; cursor:pointer;"><i class="fas fa-file-prescription"></i> View Rx</button>${valText}</div>`;
            }
            let revHtml = c.review ? `<div style="flex:1; text-align:center;"><button disabled style="width:100%; background:#f1f5f9; color:#94a3b8; border:1px solid #cbd5e1; padding:10px; border-radius:10px; font-weight:800; font-size:12px;"><i class="fas fa-star"></i> Reviewed</button></div>` : `<div style="flex:1; text-align:center;"><button onclick="openReviewModal('${c.appt_id}')" style="width:100%; background:var(--warning-light); color:#b45309; border:1px solid var(--warning); padding:10px; border-radius:10px; font-weight:800; font-size:12px; cursor:pointer;"><i class="fas fa-star"></i> Rate Doctor</button></div>`;
            postConsultHtml = `<div style="display:flex; gap:10px; margin-top:12px; align-items:flex-start;">${rxHtml}${revHtml}</div>`;
        }
        ...
        <div style="background:#ffffff; border:1px solid var(--border); border-radius:16px; padding:18px; margin-bottom:15px; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:12px; border-bottom:1px solid #f1f5f9; padding-bottom:12px;">
                <div>
                    <div style="font-size:10px; color:var(--text-light); margin-bottom:2px; font-weight:700; text-transform:uppercase;">Appt ID</div>
                    <strong style="color:var(--text-main); font-size:14px; font-weight:800;">${c.appt_id}</strong>
                    <div style="margin-top:6px;"><strong style="font-size:14px; color:var(--primary);"><i class="fas fa-user-md"></i> Dr. ${c.doctor_name}</strong></div>
                </div>
                <div style="text-align:right;">
                    <span class="status-badge ${badgeClass}" style="margin-bottom:8px;">${statusText.toUpperCase()}</span><br>
                    <span style="font-size:11px; color:var(--text-light); font-weight:700;"><i class="far fa-calendar-alt"></i> ${c.appt_date} | ${c.appt_time}</span>
                </div>
            </div>
            <div style="display:flex; justify-content:space-between; font-size:12px; color:var(--text-main); background:var(--bg-color); padding:12px; border-radius:10px; font-weight:600; border:1px solid var(--border);">
                <div><strong>Type:</strong> ${typeBadge}</div>
                <div><strong>Fee Paid:</strong> ₹${c.fee_paid} <del style="font-size:10px; color:var(--text-light); margin-left:3px; font-weight:500;">₹${c.total_mrp}</del></div>
            </div>
            ${joinBtnHtml}
            ${postConsultHtml}
            ${cancelBtnHtml}
...
async function submitCancelConsult() {
    ...
    try {
        ...
        const response = await fetch(GOOGLE_SCRIPT_URL, { method: "POST", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify(payload) });
        const result = await response.json();
        if (result.status === "success") { showToast("Appointment cancelled successfully.", "success"); document.getElementById("cancel-consult-modal").style.display = "none"; fetchPatientConsults(localStorage.getItem("bhavya_user_id")); checkLoginAndFetchData(); } 
        else { showToast("Error: " + result.message, "error"); }
...
function setRating(stars) {
    ...
    for(let i=0; i<icons.length; i++) { if(i < stars) { icons[i].classList.add("active"); icons[i].style.color = "var(--warning)"; } else { icons[i].classList.remove("active"); icons[i].style.color = "#e2e8f0"; } }
}
...
function renderPrescriptionsTab(consults) {
    ...
    if (rxConsults.length === 0) { container.innerHTML = `<div style="text-align: center; padding: 40px; color: #94a3b8; font-weight:600; background:white; border-radius:15px; border:1px solid #e2e8f0;"><i class="fas fa-file-excel" style="font-size: 30px; margin-bottom: 10px;"></i><p>No prescriptions available yet.</p></div>`; return; }

    let html = "";
    rxConsults.forEach(c => {
        let valText = c.validity_days ? `<span style="font-size:10px; background:var(--warning-light); color:#b45309; padding:3px 8px; border-radius:6px; margin-left:8px; font-weight:800;">Valid: ${c.validity_days}</span>` : "";
        html += `
        <div class="list-item" style="border-bottom:1px solid var(--border); padding:15px 0;">
            <div style="flex:1;"><h5 style="margin:0 0 5px 0; color:var(--text-main); font-size:14px; font-weight:800;"><i class="fas fa-file-medical" style="color:var(--primary); margin-right:5px;"></i> Dr. ${c.doctor_name}</h5><p style="margin:0; font-size:12px; color:var(--text-light); font-weight:600;"><i class="far fa-calendar-alt"></i> ${c.appt_date} | ${c.consult_type} ${valText}</p></div>
            <button onclick="window.open('${c.prescription_link}', '_blank')" style="background:var(--success-light); color:#065f46; border:none; padding:10px 15px; border-radius:10px; font-weight:800; font-size:12px; cursor:pointer; transition:0.2s;"><i class="fas fa-download"></i></button>
        </div>`;
    });
    container.innerHTML = `<div style="background:#fff; padding:0 20px; border-radius:16px; border:1px solid var(--border); box-shadow:0 4px 15px rgba(0,0,0,0.02);">${html}</div>`;
}
...
function updateRecentActivityMixed() {
    ...
    globalConsultsData.forEach(c => {
        ...
        allActivity.push({ type: 'doctor', id: c.appt_id, title: "Dr. " + c.doctor_name, dateTime: c.appt_date + " | " + c.appt_time, badgeClass: badgeClass, statusText: statusText, icon: '<i class="fas fa-user-md" style="color:#059669; font-size:20px;"></i>', bg: 'var(--success-light)', timestamp: new Date(c.appt_date.split("-").reverse().join("-")).getTime() || 0 });
    });
    ...
    globalMedicineOrders.forEach(med => {
        ...
        allActivity.push({ 
            ...
            icon: '<i class="fas fa-pills" style="color:#0284c7; font-size:20px;"></i>', bg: '#e0f2fe',
                ...
                if (document.getElementById("patient-ready-modal").style.display !== "block") {
                    document.getElementById("patient-ready-modal").innerHTML = `
                        <div style="text-align: center; padding: 20px;">
                            ...
                            <div style="background:var(--success-light); width:80px; height:80px; border-radius:50%; display:flex; align-items:center; justify-content:center; margin: 0 auto 15px auto;"><i class="fas fa-video" style="font-size: 35px; color: #059669; animation: pulse 1.5s infinite;"></i></div>
                            <h3 style="margin:0; color: var(--text-main); font-size: 22px; font-weight:800;">Doctor is Ready!</h3>
                            <p style="color:var(--text-light); font-size:14px; margin-top:10px; font-weight:500;">Your doctor is waiting in the clinic room. Please confirm your presence.</p>
                            <button style="background:var(--success); width:100%; color:white; border:none; font-weight:800; padding:15px; border-radius:12px; cursor:pointer; margin-top:20px; font-size: 16px; box-shadow:0 4px 15px rgba(16,185,129,0.3);" onclick="patientConfirmsReady()">I Am Ready</button>
                        </div>
                    `;
...
function joinVideoCall(link, apptId) {
    ...
    modal.innerHTML = `
        <div style="height:60px; background:var(--primary); color:white; display:flex; justify-content:space-between; align-items:center; padding:0 20px;">
            <h3 style="margin:0; font-size:16px; font-weight:800;">BhavyaCare Secure Consult</h3>
            <button onclick="closeVideoCall()" style="background:var(--danger); color:white; border:none; padding:8px 15px; border-radius:8px; cursor:pointer; font-weight:bold; font-size:13px;"><i class="fas fa-phone-slash"></i> End Call</button>
        </div>
