...
let handledPatTriggers = new Set(); // Popup loop ko rokne ke liye blocker
    ...
    consults.forEach(c => {
        ...
        if (c.appt_status === "Approved" && c.consult_type === "Online") {
            if (c.handshake_status === "Patient_Ready") {
                joinBtnHtml = `<button onclick="joinVideoCall('${c.meet_link}', '${c.appt_id}')" style="background:#fd7e14; color:white; border:none; padding:10px; border-radius:8px; font-size:13px; font-weight:bold; cursor:pointer; width:100%; margin-top:10px; animation: pulse 1.5s infinite;">📹 Re-Join Video Call</button>`;
            } else {
                joinBtnHtml = `<div style="text-align:center; font-size:11px; color:#888; margin-top:10px; padding: 10px; background: #f8f9fa; border-radius: 8px;">Call button will appear when Doctor is ready.</div>`;
            }
        } 
...
function checkPatientHandshakeTrigger() {
    globalConsultsData.forEach(c => {
        if (c.appt_status === "Approved" && c.consult_type === "Online") {
            // Check Handshake status and Local Blocker
